...
ff_data <- data %>% 
  select(q11) %>% 
  filter(grepl("^[0-9]+$", q11)) %>% 
  mutate(q11 = as.numeric(q11))

mean(ff_data$q11)
  
addmargins(table(ff_data$q11), margin = 1, FUN = sum) # Mean Ranking Results
```


<!--Remove This for REPO--
...
cord_data <- survey_rawdata %>% 
  select(Q15, LocationLatitude, LocationLongitude) %>% 
  filter(!is.na(Q15)) %>%
  slice(-c(1, 2)) %>%       # Remove the first 2 rows
  filter(str_detect(Q15, "@")) # Keep rows where 'a' contains the '@' symbol

# Sample list of coordinates (longitude, latitude)
coordinates <- data.frame(
  lon = c(-77.0365, -118.2437, -3.7038),
  lat = c(38.8977, 34.0522, 40.4168)
)

# Function to check where each coordinate falls
usa_filter <- map.where("usa", cord_data$LocationLongitude, cord_data$LocationLatitude)

# Filter only coordinates within the USA (non-NA values)
coordinates_in_usa <- cord_data[!is.na(usa_filter), ]

# Print the filtered coordinates
print(coordinates_in_usa)

write_xlsx(coordinates_in_usa, path = "emails_in_usa.xlsx")
```
